#!/usr/bin/env bash
# Pack 830demo/skill_3_pico_place_the_basket → unified (dex3 + sonic v1.1), allowlist, dual VLM cache.
#
#   bash tools/data/run_830_skill3_pico_place_basket_pack_and_cache.sh
#   SKIP_PACK=1 bash ...   # resume from existing unified
#   SKIP_CACHE=1 bash ...  # pack only
set -euo pipefail
PHI0_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
# shellcheck source=/dev/null
source "${PHI0_ROOT}/tools/env/setup_env.sh"

RAW_ROOT="${RAW_ROOT:-/mnt/data2/wpy/workspace/830demo/skill_3_pico_place_the_basket}"
MANIFEST="${MANIFEST:-${RAW_ROOT}/skill_3.json}"
OUT_DIR="${OUT_DIR:-/mnt/data3/wpy/datasets/830/830demo_skill3_pico_place_basket_unified}"
TASK_PROMPT="${TASK_PROMPT:-把篮子放到指定位置。}"
NVME_DIR="${NVME_DIR:-${OUT_DIR}}"
WS_LINK="${WS_LINK:-/mnt/data2/wpy/workspace/local_nvme/datasets/830/830demo_skill3_pico_place_basket_unified}"
CACHE_ROOT="${CACHE_ROOT:-${NVME_DIR}}"
LOG_DIR="${LOG_DIR:-${PHI0_ROOT}/logs/830_skill3_pico_place_basket_pack_cache_$(date +%Y%m%d_%H%M%S)}"
...
if [[ ! -f "${MANIFEST}" ]]; then
  echo "[830_skill3_pico] missing manifest ${MANIFEST}" >&2
  exit 1
fi

if [[ "${SKIP_PACK:-0}" != "1" ]]; then
  echo "[830_skill3_pico] pack raw=${RAW_ROOT} → ${OUT_DIR}"
...
echo "[830_skill3_pico] write allowlists"
"${PHI0_PY}" - <<PY
import json
...
echo "[830_skill3_pico] ws_link=${WS_LINK}"

if [[ "${SKIP_CACHE:-0}" == "1" ]]; then
  echo "[830_skill3_pico] SKIP_CACHE=1 — pack done, no VLM encode"
  exit 0
fi

echo "[830_skill3_pico] dual VLM frame cache phys GPUs=${CUDA_CACHE} n=${NGPU_CACHE} batch=${BATCH_SIZE}"
IFS=',' read -r -a _PHYS_GPUS <<< "${CUDA_CACHE}"
if [[ "${#_PHYS_GPUS[@]}" -ne "${NGPU_CACHE}" ]]; then
  echo "[830_skill3_pico] CUDA_CACHE must list ${NGPU_CACHE} GPUs (got ${CUDA_CACHE})" >&2
  exit 1
fi
...
for i in $(seq 0 $((NGPU_CACHE - 1))); do
  ...
  echo "[830_skill3_pico] shard ${i} -> GPU ${phys} log=${sl}"
...
echo "[830_skill3_pico] cache done on ${NVME_DIR}"
