...
Also exports state_root_trans_world, target_root_trans_world, betas, ego video.
Root xyz from observation.base_trans (g1_debug base_trans_measured); legacy parquets fall back to smpl pelvis.
``460:463`` projected_gravity_xyz from observation.projected_gravity (world [0,0,-1] in body frame).
CODE_VERSION v3.0.
"""
...
from phi0.data.projected_gravity import attach_projected_gravity_to_parquet_rows  # noqa: E402
...
class XperienceUnifiedLeRobotConverter:
    ...
    def __init__(self, fps: int):
        ...
        self.episode_sources: list[tuple[int, Path, Path, int]] = []
    ...
    def make_one_episode(
        ...
        attach_projected_gravity_to_parquet_rows(rows, groot_rows)
    ...
    def write_meta(self, out_dir: Path) -> None:
        ...
        dataset_cursor = 0
        ep_rows_meta = []
        for (_task_idx, _pq, _vid, ep_index) in sorted(self.episode_sources, key=lambda x: x[3]):
