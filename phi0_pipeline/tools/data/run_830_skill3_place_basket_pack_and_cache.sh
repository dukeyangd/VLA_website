#!/usr/bin/env bash
# DEPRECATED: non-pico skill_3_place_the_basket was wrong for 830 mix.
# Prefer: tools/data/run_830_skill3_pico_place_basket_pack_and_cache.sh
#
#   bash tools/data/run_830_skill3_place_basket_pack_and_cache.sh
...
# Default now points at pico (same as run_830_skill3_pico_place_basket_pack_and_cache.sh).
...
WS_LINK="${WS_LINK:-/mnt/data2/wpy/workspace/local_nvme/datasets/830/830demo_skill3_place_basket_unified}"
CACHE_ROOT="${CACHE_ROOT:-${NVME_DIR}}"
LOG_DIR="${LOG_DIR:-/mnt/data2/wpy/workspace/Phi_0_wpy/logs/830_skill3_place_basket_pack_cache_$(date +%Y%m%d_%H%M%S)}"
...
if [[ "${SKIP_PACK:-0}" != "1" ]]; then
  echo "[830_skill3] pack raw=${RAW_ROOT} → ${OUT_DIR}"
...
echo "[830_skill3] write allowlists"
"${PHI0_PY}" - <<PY
import json
...
echo "[830_skill3] ws_link=${WS_LINK}"

echo "[830_skill3] dual VLM frame cache phys GPUs=${CUDA_CACHE} n=${NGPU_CACHE} batch=${BATCH_SIZE}"
IFS=',' read -r -a _PHYS_GPUS <<< "${CUDA_CACHE}"
if [[ "${#_PHYS_GPUS[@]}" -ne "${NGPU_CACHE}" ]]; then
  echo "[830_skill3] CUDA_CACHE must list ${NGPU_CACHE} GPUs (got ${CUDA_CACHE})" >&2
  exit 1
fi
...
for i in $(seq 0 $((NGPU_CACHE - 1))); do
  ...
  echo "[830_skill3] shard ${i} -> GPU ${phys} log=${sl}"
...
echo "[830_skill3] cache done on ${NVME_DIR}"
