...
def _check_trainable_scope(model) -> None:
    ...
    logger.info(
        "Trainable scope OK: %d tensors under action_expert (VLM frozen).",
        len(trainable),
    )
...
def run_checks(cfg: DictConfig, *, smoke_model: bool = True, load_vlm: bool = False) -> None:
    ...
    logger.info("ACT future placeholder: zeros only (no learnable raw-space pert)")
...
@hydra.main(config_path="../../configs", config_name="train_xperience_unified", version_base="1.3")
