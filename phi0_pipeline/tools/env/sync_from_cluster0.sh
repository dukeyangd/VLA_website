...
_sync_agent_sim() {
  ...
  bash "${ROOT}/tools/env/fix_venv_sim.sh" >> "${LOG_DIR}/${p}_fix_venv_sim.log" 2>&1 || true
  echo "[stub] LeRobot video placeholders (ep447 mp4 real, others empty)"
  "${PHI0_PY}" "${ROOT}/tools/data/ensure_pick_tissue_video_stubs.py" \
