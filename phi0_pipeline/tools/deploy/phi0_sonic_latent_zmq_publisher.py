...
def parse_args():
    ...
    # RTC flags — override / follow model cfg rtc.* (default: yaml enabled)
    p.add_argument(
        "--rtc",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Deploy RTC hard frozen-prefix (default: model.rtc.enabled / USE_RTC)",
    )
...
def _send_deploy_arm_commands(pub: zmq.Socket) -> None:
...
def _predict_motion_deploy_rtc(
    ...
    *,
    ...
    if chunk_h is None:
        chunk_h = resolve_deploy_action_chunk_size(model, seq_len=seq_len)
    else:
        chunk_h = int(chunk_h)
...
def _run_model_inference(
    *,
    ...
    if rtc_cfg and rtc_cfg.get("enabled"):
        from phi0.inference.rtc import rtc_deploy_params_ok

        d = int(rtc_cfg["inference_delay"])
        s = int(rtc_cfg["execution_horizon"])
        if not rtc_deploy_params_ok(chunk_h, d, s):
            logger.warning(
                "RTC deploy disabled: need d<=s<=H-d, got d=%d s=%d H=%d",
                d,
                s,
                chunk_h,
            )
            rtc_cfg = {**rtc_cfg, "enabled": False}
...
def _stream_over_zmq(
    *,
    ...
    # Locked: ARM(+start_streamed) before ENTER; READY only gates token frames.
    if stream_now:
...
def main() -> None:
    ...
    if precompute_in is not None:
        ...
        # past_w=1 + seq_len=33 → H=32; soft-disable invalid d/s before load.
        _seq = int(cfg.data.get("seq_len", 33))
        _model = getattr(cfg, "model", cfg)
        _pw = int(getattr(_model, "past_action_window_size", None) or cfg.get("past_action_window_size", 1) or 1)
        rtc_cfg = _resolve_rtc_cfg(cfg, args, horizon=max(1, _seq - _pw))
