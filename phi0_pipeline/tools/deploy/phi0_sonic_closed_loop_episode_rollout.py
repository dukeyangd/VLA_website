...
for p in (ROOT / "src", ROOT / "tools" / "train"):
...
class EpisodeRolloutConfig:
    ...
    config_name: str = "train_pick_yellow_box_xperience_unified_ddp4_16k"
    prompt: str = "pick tissue"
    gt_repo_id: str = "pick_tissue_xperience_unified"
    device: str = "cuda"
    ...
    print_start_prefix: bool = False
    print_start_prefix_full: bool = False
    prefix_dump_path: str = ""
    """Log prefix once on first predict."""
    print_dataset_body_xyz: bool = False
    """Log dataset unified_action[360:363] at each inference."""
...
def main(config: EpisodeRolloutConfig) -> None:
    ...
    args = SimpleNamespace(
        ...
        gt_repo_id=str(config.gt_repo_id),
        camera_source="gt",
        ...
        vlm_input="hwc",
        benchmark_vlm_input=False,
        print_start_prefix=bool(config.print_start_prefix),
        print_start_prefix_full=bool(config.print_start_prefix_full),
        prefix_dump_path=str(config.prefix_dump_path),
        print_dataset_body_xyz=bool(config.print_dataset_body_xyz),
    )
