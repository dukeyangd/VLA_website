#!/usr/bin/env python3
"""Agent -> skill -> SONIC latent ZMQ sim (ep447 ego). stay skips deploy."""
...
def _load_episode_images(episode_idx: int):
    ...
    ego, clip_row, _task = load_pick_tissue_episode_images(episode_idx)
    return ego, clip_row
...
def _run_sonic_sim(
    *,
    ...
    script = ROOT / "tools/eval/run_sonic_latent_sim_eval.sh"
...
def main():
    ...
    ego, clip_row = _load_episode_images(int(args.episode_idx))
    ...
    if args.force_skill:
        ...
        agent_result = agent.run(args.user_instruction, ego)
