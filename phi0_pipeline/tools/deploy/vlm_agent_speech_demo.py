...
def _obs_video_bcthw(cpu_payload, *, device, dtype) -> torch.Tensor:
    obs = cpu_payload["obs_pixel"]  # [B,1,C,H,W] in [0,1]
    return _frame_to_bcthw(obs[:, 0], device=device, dtype=dtype)
...
def main():
    ...
    video = _obs_video_bcthw(cpu_payload, device=model.device, dtype=model.torch_dtype)
