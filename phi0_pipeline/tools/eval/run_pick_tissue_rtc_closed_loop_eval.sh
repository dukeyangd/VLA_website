...
#
# Usage:
#   bash tools/eval/run_pick_tissue_rtc_closed_loop_eval.sh
#   bash tools/eval/run_pick_tissue_rtc_closed_loop_eval.sh 447 logs/my_run
set -euo pipefail
...
echo "[rtc_eval] inference_rate=${INFERENCE_RATE}Hz (0=prefetch ASAP) motion_seconds=${MOTION_SECONDS}"
...
CHECKPOINT="${CHECKPOINT}" CONFIG_NAME="${CONFIG_NAME}" \
  ...
  PRINT_START_PREFIX="${PRINT_START_PREFIX}" PRINT_START_PREFIX_FULL="${PRINT_START_PREFIX_FULL}" \
  PREFIX_DUMP_PATH="${PREFIX_DUMP_PATH:-${RUN_DIR}/start_prefix.json}" \
