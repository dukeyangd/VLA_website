#!/usr/bin/env bash
# Student ẑ + hand_pred → in-tree Revo2 MuJoCo for 820 skill_1/2/3.
# Not Newton-GL. Not efs replay_gt_latent.sh.
#
#   STUDENT_CKPT=.../phi0_student_last.pt bash tools/eval/run_student_skill123_mujoco_viz.sh
set -euo pipefail
...
TAG="${TAG:-student_skill123_$(date +%Y%m%d_%H%M%S)}"
OUT="${OUT:-${PHI0_ROOT}/experiments/${TAG}}"
MAX_FRAMES="${MAX_FRAMES:-579}"
HORIZON="${HORIZON:-16}"
...
mkdir -p "${OUT}"
echo "[skill123] ckpt=${STUDENT_CKPT}"
echo "[skill123] out=${OUT} frames=${MAX_FRAMES} H=${HORIZON}"

for s in 1 2 3; do
  REF="/mnt/data2/wpy/workspace/820demo/820demo_skill_${s}_release_unified"
  SD="${OUT}/skill${s}"
  mkdir -p "${SD}/logs"
  INFER_NPZ="${SD}/infer_qpos_traj_student.npz"
  TOKENS="${SD}/tokens.npz"
  MOTION="${SD}/motion_v4.npz"
  MP4="${SD}/gt_latent_revo2/student_sonic_revo2.mp4"

  echo "[skill123] === skill${s} offline infer ==="
  ...
  echo "[skill123] === skill${s} pack tokens ==="
...
PY
  ...
  echo "[skill123] === skill${s} MuJoCo Revo2 replay ==="
  ...
  echo "[skill123] skill${s} mp4 → ${MP4}"
  ls -lh "${MP4}" || true
done

echo "[skill123] DONE out=${OUT}"
ls -lh "${OUT}"/skill*/gt_latent_revo2/*.mp4 2>/dev/null || true
