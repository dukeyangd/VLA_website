#!/usr/bin/env bash
# Student ẑ → MuJoCo Revo2 for 820 mix v2: demo5 (5) + teleop skills (5).
# Ref/prompt/offsets from 820demo_mix_v2_release_unified (train-aligned).
#
#   STUDENT_CKPT=.../phi0_student_last.pt HORIZON=32 \
#     bash tools/eval/run_student_mix_v2_all_mujoco_viz.sh
set -euo pipefail
...
mkdir -p "${OUT}"
echo "[mix_v2] ckpt=${STUDENT_CKPT}"
echo "[mix_v2] out=${OUT} H=${HORIZON} ref=${REF}"

# file tag ep ref_start frames prompt_prefix (log grep for demo5 only)
SKILLS=(
  ...
  "file-005.parquet skill1 5 1623 579 机器人朝黑箱子"
  "file-226.parquet place_basket 226 117716 1430 把篮子放到指定"
  "file-431.parquet give_me_five 431 398384 2129 走上前和人击掌"
  "file-531.parquet pick_rubbish 531 585771 1784 捡起地上的垃圾"
  "file-715.parquet skill_0 715 810459 473 旋转朝向"
)
...
run_one() {
  ...
  echo "[mix_v2] === ${tag} offline infer start=${start} frames=${n} ==="
  ...
  if [[ -n "${prefix}" ]] && ! grep -q "prompt0=${prefix}" "${SD}/logs/offline_infer.log"; then
    echo "[mix_v2] WARN ${tag}: expected prompt0=${prefix}"
    grep -E 'prompt0=|no mp4|Error' "${SD}/logs/offline_infer.log" | tail -5 || true
  fi

  echo "[mix_v2] === ${tag} pack ==="
...
PY
  ...
  echo "[mix_v2] === ${tag} MuJoCo Revo2 replay ==="
